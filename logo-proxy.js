...
// Port PUBLIK khusus NBI/REST API (opsional). Terpisah dari port dashboard
// (RADFAST_PROXY_PORT) supaya integrasi billing punya endpoint sendiri.
// PENTING: port ini TETAP mewajibkan secret path (RADFAST_NBI_GATE_PATH) —
// NBI asli tetap di-bind 127.0.0.1 dan tidak pernah diexpose tanpa secret.
// 0 / kosong = fitur dimatikan (default), cukup pakai gate di port dashboard.
const NBI_PROXY_PORT = parseInt((process.env.RADFAST_NBI_PROXY_PORT || '').trim() || '0', 10) || 0;
...
(function(){
  ...
  function apiUrlFromPath(path, port){
    var host=location.hostname;
    if(port && parseInt(port,10)>0){ host=host+':'+parseInt(port,10); }
    else if(location.port){ host=host+':'+location.port; }
    return location.protocol+'//'+host+(path||'');
  }
  // URL lengkap siap-copy ke RadFastBill (langsung ke endpoint /devices)
  function apiUrlFull(path, port){ return path ? (apiUrlFromPath(path, port)+'/devices') : ''; }
  ...
  function buildApiModal(){
    ...
    function render(d){
      ...
      url.textContent=d.secretPath?apiUrlFull(d.secretPath, d.nbiProxyPort):'(NBI gate nonaktif)';
      ...
      var note=body.querySelector('#rf-api-note');
      if(note){
        var portTxt=(d.nbiProxyPort && parseInt(d.nbiProxyPort,10)>0)
          ? 'URL pakai port khusus REST API: <b>'+parseInt(d.nbiProxyPort,10)+'</b> (terpisah dari port dashboard).'
          : 'URL pakai port dashboard. Set <code>RADFAST_NBI_PROXY_PORT</code> di .env kalau mau port REST API terpisah.';
        note.innerHTML='Ganti secret maksimal 3× per restart instance. Setelah secret diganti, update URL ini di RadFastBill.<br>'+portTxt;
      }
...
// ════════════════════════════════════════════════════════════
//  NBI PUBLIC PROXY (opsional) — port terpisah khusus REST API
//  Aktif hanya jika RADFAST_NBI_PROXY_PORT di-set di .env.
//  Endpoint billing: http://<ip>:<NBI_PROXY_PORT>/<secret>/devices
//  Keamanan TETAP: wajib secret path; tanpa secret → 403. NBI asli
//  tetap di 127.0.0.1 dan tidak pernah diexpose langsung.
// ════════════════════════════════════════════════════════════
if (NBI_PROXY_PORT) {
    if (NBI_PROXY_PORT === PUBLIC_PORT) {
        console.error(`[nbi-proxy] SKIP: RADFAST_NBI_PROXY_PORT (${NBI_PROXY_PORT}) sama dengan port dashboard. Pakai port lain.`);
    } else {
        const nbiServer = http.createServer((req, res) => {
            const ip = getIP(req);
            ...
            // Batasi ukuran body sebelum dibaca
            if (req.method === 'POST') {
                const cl = parseInt(req.headers['content-length'] || '0', 10);
                if (cl > MAX_SIZE + 65536) {
                    auditLog('NBI-PROXY-REJECT', ip, `Content-Length terlalu besar: ${cl} bytes`);
                    sendSecure(res, 413, 'text/plain', 'Payload Too Large');
                    return;
                }
            }

            // Wajib lewat SECRET PATH — tanpa itu tidak diteruskan ke NBI.
            if (NBI_GATE_PATH &&
                (req.url === NBI_GATE_PATH ||
                 req.url.startsWith(NBI_GATE_PATH + '/') ||
                 req.url.startsWith(NBI_GATE_PATH + '?'))) {
            ...
            // Selain secret path → tolak (port ini KHUSUS NBI, bukan UI/admin).
            auditLog('NBI-PROXY-BLOCK', ip, `${req.method} ${req.url} — tanpa secret path`);
            sendSecure(res, 403, 'application/json',
                '{"error":"Forbidden — gunakan secret path untuk akses REST API"}');
        });

        nbiServer.on('error', err => {
            if (process.env.RADFAST_MULTI_PROXY === '1') {
                console.error(`[nbi-proxy] :${NBI_PROXY_PORT} SKIP (${err.code || err.message})`);
                return;
            }
            console.error('[nbi-proxy] Fatal:', err.message);
        });

        nbiServer.listen(NBI_PROXY_PORT, '::', () => {
            console.log(`[nbi-proxy] :${NBI_PROXY_PORT} → NBI :${NBI_PORT || '(belum diset)'} (wajib secret path)`);
        });
    }
}
